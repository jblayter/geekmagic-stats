...
- **Claude Desktop fallback:** if the token is dead and can't be refreshed, but the **Claude Desktop app** is running, the screen reads its local usage log (`~/Library/Application Support/Claude/plan-usage-history.json`, updated every ~5 min) and shows **Session/Weekly from there** — no credentials needed. This mode is partial: it has only the two headline percentages, so there are no reset countdowns, pace markers, or Fable weekly limit, and the header shows a small amber **`desktop`** tag to signal it. Only the plain usage file is read — never the app's encrypted cookie/token stores.
- **Failure cards:** if none of the above yields data, a transient network/API error shows a neutral **"Not connected"** card that self-heals on the next fetch. If the problem is authentication (expired/missing token or an unrecoverable `401`), the screen shows a **re-login card** — "Login expired" with the exact steps (run `claude`, then `/login` if prompted).

The fallback order is: fresh cache → live API (refreshing the token if needed) → recent cache (< 30 min) → Claude Desktop sample (< 15 min) → failure card.
