//! `geekmagic-ci` — render the CI runners screen to a GeekMagic display.

use anyhow::{anyhow, Result};
use clap::Parser;

use geekmagic_common::{ci, config, upload};

#[derive(Parser)]
#[command(about = "Render self-hosted CI runner status to a GeekMagic display")]
struct Args {
    #[arg(long)]
    host: Option<String>,
    ...
    /// Render representative sample data instead of querying Docker/GitHub
    #[arg(long)]
    demo: bool,
}
...
fn main() -> Result<()> {
    ...
    upload::upload_named(&host, "ci-runners.jpg", &img)?;
