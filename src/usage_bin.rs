//! `geekmagic-usage` — render the per-model usage screen (Fable 5 featured).

use anyhow::{anyhow, Result};
use clap::Parser;

use geekmagic_common::{config, upload, usage};

#[derive(Parser)]
#[command(about = "Render per-model Claude usage (Fable 5 featured) to a GeekMagic display")]
struct Args {
    #[arg(long)]
    host: Option<String>,
...
fn main() -> Result<()> {
    ...
    // When uploading, verify the device is reachable before doing scan work.
    ...
    upload::upload_named(&host, "model-usage.jpg", &img)?;
